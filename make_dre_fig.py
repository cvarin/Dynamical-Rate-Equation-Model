...
if __name__ == '__main__':
	...
	params_abc = [10*fs, 1.75e4, 1000]
	params_def = [300*fs, 6.76e4, 3000]
	...
	for i, params in enumerate([params_abc, params_def]):
		...
		ax.semilogy(t_axis, results["rho"]*results["xi_e"]/mat.density, label=r"$\xi\rho$", color="darkred", lw=lw)
		...
		Ekinmax = el_Ekin_max(results['E'], mat, las)
		# print(Ekinmax/eV, results["Ekin"].max()/eV)
		ax2.semilogy(t_axis, results["Ekin"]/eV, color=colors[2], label=r"$\mathcal{E}_\mathrm{kin}$", lw=lw)
		...
		plt.semilogy(t_axis, results["coll_freq_en"]*fs, label=r"$\gamma_{u}$", color=colors[2], lw=lw)
		plt.semilogy(t_axis, results["el_heating_rate"]*fs, label=r"$\gamma_\mathrm{lh}$", color="darkred", lw=lw)
	...
	plt.savefig("dre_fig.pdf")
	# os.system("pdfcrop dre_fig.pdf dre_fig.pdf > /dev/null")
